...
사용법:
    ...
    # 단일 웹캠 (우측)
    python3 test_webcam_stream.py right 0 http://100.64.1.1:5000
...
파라미터:
    camera_id     "left", "right", 또는 "dual"
    camera_index  웹캠 장치 인덱스 (보통 0 또는 1)
    server_url    Flask 서버 URL (기본값: http://100.64.1.1:5000)
...
def main():
    ...
    # 기본 서버 URL
    default_server_url = os.getenv('SERVER_URL', 'http://100.64.1.1:5000')
    ...
    if mode in ['left', 'right']:
        ...
        streamer.stream_single_camera(camera_id, camera_index)
        ...
        left_index = int(sys.argv[2])
        right_index = int(sys.argv[3])
        server_url = sys.argv[4] if len(sys.argv) > 4 else default_server_url

        streamer = WebcamStreamer(server_url)
        streamer.stream_dual_camera(left_index, right_index)
