#!/usr/bin/env python3
"""
18개 Through-hole 클래스 데이터 증강 스크립트

목적:
- 객체 수가 적은 클래스를 증강하여 균형 맞추기
- 목표: 각 클래스당 최소 200개 객체 (Train 세트)

증강 기법:
- Horizontal Flip
- Vertical Flip
- Rotation (90도, 180도, 270도)
...
# 경로 설정
SOURCE_DIR = "/home/sys1041/work_project/data/processed/throughhole_18classes"
OUTPUT_DIR = "/home/sys1041/work_project/data/processed/throughhole_18classes_augmented"
...
CLASS_NAMES = [
    ...
    'Capacitor-470mf',                   # 4
    'Diode',                             # 5
    'High-Voltage-Ceramic-Capacitor',    # 6
    'IGBT',                              # 7
    'Inductor',                          # 8
    'LED-Light',                         # 9
    'Low-Voltage-Ceramic-Capacitor',     # 10
    'MLC-Capacitor',                     # 11
    'MOSFET',                            # 12
    'NTC-Thermistor',                    # 13
    'Push-Switch',                       # 14
    'Tact-Switch',                       # 15
    'Trimmer-Potentiometer',             # 16
    'Zener-Diode'                        # 17
]
...
def main():
    print("=" * 100)
    print("18개 Through-hole 클래스 데이터 증강")
    print("=" * 100)
    ...
    print(f"\n{'클래스 ID':<12} {'클래스 이름':<35} {'현재 객체':>12} {'목표':>10} {'증강 필요':>12}")
    print("-" * 100)
    ...
    for class_id in range(18):
        ...
        if need_augment > 0:
            status = f"⚠️ +{need_augment}"
        else:
            status = "✅"

        print(f"{class_id:<12} {class_name:<35} {current_count:>12} {TARGET_MIN_OBJECTS:>10} {status:>12}")
    ...
    # 증강 후 클래스별 객체 수
    print("\n📊 증강 후 Train 세트 클래스별 객체 수:")

    # 임시로 증강된 데이터셋에서 객체 수 카운트
    aug_train_counts = Counter()
    aug_train_lbl_dir = os.path.join(OUTPUT_DIR, 'train', 'labels')

    for label_file in os.listdir(aug_train_lbl_dir):
        if not label_file.endswith('.txt'):
            continue

        with open(os.path.join(aug_train_lbl_dir, label_file), 'r') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) >= 5:
                    class_id = int(parts[0])
                    aug_train_counts[class_id] += 1

    print(f"\n{'클래스 ID':<12} {'클래스 이름':<35} {'이전':>10} {'증강 후':>10} {'증가':>10}")
    print("-" * 100)

    for class_id in range(18):
        class_name = CLASS_NAMES[class_id]
        before_count = train_counts.get(class_id, 0)
        after_count = aug_train_counts.get(class_id, 0)
        ...
        print(f"{class_id:<12} {class_name:<35} {before_count:>10} {after_count:>10} {status:>10}")
