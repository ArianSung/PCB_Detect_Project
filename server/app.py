    ...
    try:
        ...
        if serial_detector is not None:
            try:
                ...
                # 디버그: OCR 전처리된 이미지를 파일로 저장 (사용자 확인용)
                if ocr_processed_image is not None:
                    try:
                        cv2.imwrite('/tmp/ocr_debug.jpg', ocr_processed_image)
                        logger.info(f"[OCR-DEBUG] 전처리 이미지 저장: /tmp/ocr_debug.jpg (shape: {ocr_processed_image.shape})")
        ...
        # 8. 최종 판정
        # 시리얼 넘버 없거나 ROI 밖이면 판정 = None
        if not serial_number:
            decision = None
            gpio_pin = 23  # 기본값
            logger.warning("⚠️ 시리얼 넘버 없음 → 판정 = None")
        elif roi_status != 'in_roi':
            decision = None
            gpio_pin = 23
            logger.warning(f"⚠️ ROI 밖 (상태: {roi_status}) → 판정 = None")
        elif not verification_possible:
            decision = None
            gpio_pin = 23
            logger.warning("⚠️ 검증 불가능 → 판정 = None")
        elif missing_count >= 3 or position_error_count >= 5 or (missing_count + position_error_count) >= 7:
            decision = "discard"
            gpio_pin = 22
