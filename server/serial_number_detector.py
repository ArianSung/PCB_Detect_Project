...
class SerialNumberDetector:
    ...
    def parse_serial_number(self, text: str) -> Optional[Tuple[str, str, str]]:
        ...
        def clean_ocr_text(raw_text: str) -> str:
            ...
            # ⚠️ 중요: upper() 하기 전에 소문자 o→0 치환 필수!
            cleaned = raw_text

            # **OCR 오인식 문자 치환** (소문자/대문자 모두)
            cleaned = cleaned.replace('o', '0')  # 소문자 o → 0 ⭐
            cleaned = cleaned.replace('O', '0')  # 대문자 O → 0
            cleaned = cleaned.replace('i', '1')  # 소문자 i → 1
            cleaned = cleaned.replace('I', '1')  # 대문자 I → 1
            cleaned = cleaned.replace('l', '1')  # 소문자 l → 1
            cleaned = cleaned.replace('L', '1')  # 대문자 L → 1

            # 이제 대문자 변환 (알파벳만 대문자로, 숫자는 그대로)
        ...
        def normalize_serial(serial_num: str, product_code: str) -> Optional[str]:
            ...
            """
            # **OCR 오인식 문자 치환** (O/o → 0, I/l → 1, 등)
            # 시리얼 넘버는 숫자만 포함하므로 알파벳을 숫자로 변환
            # ⚠️ 중요: 소문자 치환을 먼저 해야 함 (upper() 이전)
            corrected = serial_num
            corrected = corrected.replace('o', '0')  # 소문자 o → 숫자 0 ⭐
            corrected = corrected.replace('O', '0')  # 대문자 O → 숫자 0
            corrected = corrected.replace('Q', '0')  # 알파벳 Q → 숫자 0
            corrected = corrected.replace('i', '1')  # 소문자 i → 숫자 1
            corrected = corrected.replace('I', '1')  # 대문자 I → 숫자 1
            corrected = corrected.replace('l', '1')  # 소문자 l → 숫자 1
            corrected = corrected.replace('L', '1')  # 대문자 L → 숫자 1
            corrected = corrected.replace('Z', '2')  # 알파벳 Z → 숫자 2
            corrected = corrected.replace('z', '2')  # 소문자 z → 숫자 2
            corrected = corrected.replace('S', '5')  # 알파벳 S → 숫자 5
            corrected = corrected.replace('s', '5')  # 소문자 s → 숫자 5
            corrected = corrected.replace('G', '6')  # 알파벳 G → 숫자 6
            corrected = corrected.replace('g', '6')  # 소문자 g → 숫자 6
            corrected = corrected.replace('B', '8')  # 알파벳 B → 숫자 8
            corrected = corrected.replace('b', '8')  # 소문자 b → 숫자 8

            # 숫자만 추출 (하이픈, 공백, 남은 알파벳 모두 제거) ⭐
