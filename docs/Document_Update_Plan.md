...
- **신규**: 이중 전문 YOLO 모델
  - 모델 1: custom_component (자체 촬영 부품 데이터셋, 6-8개 클래스)
  - 모델 2: custom_solder (자체 촬영 납땜 데이터셋, 5-6개 클래스)

### 데이터셋 변경
- **기존**: 병합 데이터셋 (29개 클래스, 심각한 불균형)
- **신규**:
  - custom_component: 실제 웹캠 촬영 원본 200-300장 (증강 후 600-900장)
  - custom_solder: 실제 웹캠 촬영 원본 200-300장 (증강 후 600-900장)
...
**수정 항목**:
- [ ] 기존 공개 데이터셋 설명 아카이브 처리
- [ ] custom_component 촬영·라벨링·증강 절차 상세화
- [ ] custom_solder 촬영·라벨링·증강 절차 상세화
- [ ] Roboflow Export → YOLO 형식 배치 가이드
- [ ] 클래스 정의 표 (프로젝트 기준 6-8 + 5-6개)
...
**수정 항목**:
- [ ] 이중 모델 독립 학습 가이드
- [ ] 모델 1 학습 (custom_component)
- [ ] 모델 2 학습 (custom_solder)
- [ ] 각 모델별 하이퍼파라미터
- [ ] 성능 평가 기준
...
- [ ] 새로운 데이터셋 (custom_component, custom_solder) 정확히 명시
- [ ] 이중 모델 아키텍처 일관되게 설명
