...
**목표**: 컨베이어 벨트를 통해 들어오는 PCB의 양면을 실시간으로 검사하여 제품별 부품 배치를 검증하고 불량을 자동 판정하는 AI 딥러닝 시스템 개발
...
- **WebSocket 기반 실시간 프레임 전송** 및 웹 서버 통신 (Flask-SocketIO) ⭐
  - 양방향 실시간 통신
  - 클라이언트 요청 시 프레임 전송 (백프레셔 제어)
  - 낮은 오버헤드 (~10 bytes/frame)
- 양면 순차 처리 및 제품별 검증 판정
...
- **Windows PC (C# WinForms)**
  - **WebSocket 기반 실시간 모니터링 대시보드** ⭐
    - SocketIOClient 라이브러리 사용
    - 100ms 간격으로 프레임 요청 (10 FPS)
    - 바운딩 박스 실시간 표시
  - 검사 이력 조회 및 관리
  - 불량 이미지 뷰어
  - 시스템 설정 UI
- **분류 시스템** (GPIO 제어 - **라즈베리파이 1 전용**):
  - 부품 누락 → GPIO 핀 17 (BCM) HIGH (재작업 라인)
  - 위치 오류 → GPIO 핀 27 (BCM) HIGH (재작업 라인)
  - 폐기 → GPIO 핀 22 (BCM) HIGH (폐기 라인)
  - 정상 → GPIO 핀 23 (BCM) HIGH (다음 공정)
  - **참고**: Flask 서버가 뒷면(제품 식별) + 앞면(부품 검증) 결과를 통합 판정 후, 라즈베리파이 1에만 GPIO 제어 신호 전달
...
### 1. 뒷면 처리: 제품 식별 (우측 카메라)

**시리얼 넘버 OCR**:
- 형식: MBXX12345678 (12자리)
  - MB: 브랜드명 (2자리)
  - XX: 제품 코드 (2자리 영문, 예: FT, RS 등)
  - 12345678: 일련번호 (8자리 숫자)
- 기술: YOLO 영역 검출 + EasyOCR 텍스트 인식
- 정규식 검증 및 제품 DB 조회

**QR 코드 스캔**:
- pyzbar 라이브러리를 이용한 QR 코드 디코딩
- 로컬 웹 URL 파싱 (제품 정보 페이지)
- 제품명 및 설명 정보 제공
...
### 2. 앞면 처리: 부품 위치 검증 (좌측 카메라)

**YOLO 부품 검출**:
- 커스텀 데이터셋 기반 YOLOv11l 모델
- 실제 사용 부품 클래스 검출
- 부품 중심점 좌표 계산

**부품 위치 검증** (component_verification.py):
- 검출된 부품 위치 vs 제품별 기준 위치 비교
- 유클리드 거리 기반 위치 오류 판정 (threshold: 20px)
- 누락 컴포넌트 검출
- 추가 컴포넌트 검출

### 3. 최종 판정 로직

Flask 서버에서 뒷면 + 앞면 결과를 통합하여 최종 판정:
...
### Phase 2: 커스텀 PCB 데이터 준비 (2-3주) ⭐ 업데이트

#### 데이터 수집 전략
**프로젝트 특화 커스텀 데이터셋 구축**:
1. **부품 검출 데이터셋** (앞면 촬영)
2. **시리얼 넘버 데이터셋** (뒷면 촬영, 선택 사항)
3. **제품별 부품 배치 기준 데이터**

#### 체크리스트
- [x] 데이터 수집 계획 수립
  - [x] 3종 제품별 샘플 PCB 확보
  - [x] 촬영 환경 설정 (조명, 카메라 각도)
  - [x] 데이터 수집 도구 준비
- [ ] 부품 검출 데이터셋 수집 (앞면)
  - [ ] 각 제품별 정상 샘플 촬영
  - [ ] 다양한 조명 조건에서 촬영
...
- [ ] 시리얼 넘버 데이터셋 수집 (뒷면, 선택)
  - [ ] 시리얼 넘버 영역 촬영
  - [ ] 다양한 폰트 및 위치 변형
  - [ ] 영역 라벨링
- [ ] 제품별 부품 배치 기준 생성
  - [ ] 각 제품의 정상 PCB로부터 부품 위치 추출
  - [ ] 부품 중심점 좌표 기록
  - [ ] JSON 형식으로 저장
- [ ] 데이터 품질 검증
  - [ ] 이미지 해상도 확인 (640x640 권장)
...
**참고 문서**: `Dataset_Guide.md`, `Serial_Number_Detection_Guide.md`

---

### Phase 3: 부품 검출 모델 학습 및 최적화 (3-4주) ⭐ 업데이트

**핵심 변경**: 커스텀 데이터셋 기반 단일 부품 검출 모델 학습

#### 부품 검출 모델 학습
- [x] 데이터셋: 커스텀 PCB 데이터셋 (부품 검출용)
- [ ] YOLOv11l 선택 (정확도 우선, RTX 4080 Super 최적화)
- [ ] 학습 설정
  - [ ] Epochs: 100-150
  - [ ] Batch size: 16 (VRAM 12-14GB 사용, 권장)
  - [ ] Image size: 640
  - [ ] Optimizer: AdamW
  - [ ] Learning rate: 0.001
...
- [ ] 성능 평가
  ...
  - [ ] Precision/Recall 분석
  - [ ] 제품별 성능 검증
- [ ] 모델 저장: `models/component_detector_best.pt`

#### 시리얼 넘버 검출 모델 학습 (선택 사항)
- [ ] 데이터셋: 시리얼 넘버 영역 데이터셋
- [ ] YOLOv11n 선택 (가벼운 모델, 영역 검출만)
- [ ] 학습 설정
  - [ ] Epochs: 50-100
  - [ ] Batch size: 16
  - [ ] Image size: 640
- [ ] 모델 저장: `models/serial_detector_best.pt`

#### 모델 최적화
- [ ] 하이퍼파라미터 튜닝
  - [ ] Learning rate 스케줄러
  - [ ] Warm-up epochs (10-20)
  - [ ] Weight decay 조정
- [ ] 추론 최적화
  - [ ] FP16 (Half Precision) 적용 ⭐ 권장
  - [ ] 추론 시간 최소화 (목표: 30-50ms)
  - [ ] TensorRT 변환 (선택)
- [ ] 성능 벤치마크
  - [ ] 부품 검출 추론 시간 측정
  - [ ] OCR 처리 시간 측정
...
### Phase 4: 제품 식별 시스템 구현 (QR/Serial) (2-3주) ⭐ 업데이트

**핵심 변경**: 뒷면에서 시리얼 넘버와 QR 코드로 제품을 식별하고, 제품별 부품 배치 기준으로 앞면을 검증하는 시스템 구현

#### QR 코드 검출기 구현
- [ ] pyzbar 라이브러리 설치 및 테스트
- [ ] QR 코드 디코딩 모듈 작성 (`server/qr_detector.py`)
  - [ ] 이미지에서 QR 코드 검출
  - [ ] URL 파싱 및 제품 정보 추출
  - [ ] 에러 처리 (QR 없음, 잘못된 형식 등)
- [ ] 로컬 웹 서버 구축 (제품 정보 표시)
  - [ ] Flask 기반 간단한 웹 페이지
  - [ ] 제품명, 설명, 스펙 표시
- [ ] 통합 테스트
...
#### 시리얼 넘버 OCR 시스템 구현
- [ ] EasyOCR 설치 및 환경 설정
- [ ] 시리얼 넘버 검출 모듈 작성 (`server/serial_number_detector.py`)
  - [ ] YOLO 영역 검출 (선택) 또는 직접 OCR
  - [ ] EasyOCR로 텍스트 인식
  - [ ] 정규식 검증: `^MB[A-Z]{2}\d{8}$`
  - [ ] 제품 코드 파싱 (XX 부분 추출)
- [ ] 다양한 폰트 및 조명 조건 테스트
...
#### 제품 데이터베이스 구축
- [ ] MySQL 테이블 설계
  - [ ] `products`: 제품 정보 (제품 코드, 이름, 설명)
  - [ ] `product_components`: 제품별 부품 배치 기준
- [ ] 제품 관리 모듈 작성 (`server/product_database.py`)
  - [ ] 제품 코드 기반 조회
  - [ ] 부품 배치 기준 로드
  - [ ] 제품별 메타데이터 관리
- [ ] 3종 제품 데이터 입력
  - [ ] 각 제품의 정상 PCB로부터 부품 위치 추출
  - [ ] JSON 형식으로 저장 및 DB 입력
- [ ] 데이터베이스 백업 및 복원 스크립트

**참고 문서**: `Serial_Number_Detection_Guide.md`, `MySQL_Database_Design.md`

---

### Phase 5: 제품별 검증 시스템 통합 및 웹서버 구축 (3-4주) ⭐ 업데이트

#### 실시간 검사 시스템 아키텍처

```
[컨베이어 벨트 시스템 - 양면 순차 검사]
    │
    ...
    └─ 웹캠 2 (우측) ──→ [라즈베리파이 2] (100.x.x.z)
        ...
        │  │  Flask SocketIO: ws://100.x.x.x:5000   │ │
        │  └────────────────────────────────────────┘ │
        │  - 실시간 모니터링 대시보드 (바운딩 박스)  │
        │  - 박스 상태 모니터링                       │
        │  - 제품별 검증 결과 표시 ⭐                │
        │  - 검사 이력 조회 (MySQL via REST API)     │
        │  - 로컬 웹 서버 (QR 제품 정보) ⭐           │
        │  - 시스템 설정 관리                         │
        └─────────────────────────────────────────────┘
...
### 데이터셋 ⭐ 업데이트
**사용 중인 데이터셋**:
- **커스텀 PCB 데이터셋** - 프로젝트 특화 데이터
  - 부품 검출 데이터셋 (앞면 촬영)
  - 시리얼 넘버 데이터셋 (뒷면 촬영, 선택)
  - 제품별 부품 배치 기준 데이터
  - 3종 제품 지원 (FT, RS 등)
...
**작성일**: 2025-10-28
**최종 수정일**: 2025-11-28
**버전**: 3.0 ⭐ 제품별 검증 시스템 업데이트

---

## 변경 이력

### v3.0 (2025-11-28) ⭐ 제품별 검증 시스템으로 전환
- **핵심 변경**: 이중 YOLO 모델 → 제품별 부품 배치 검증 시스템
- **뒷면 처리**: 시리얼 넘버 OCR + QR 코드 스캔으로 제품 식별
  - 시리얼 넘버 형식: MBXX12345678 (MB=브랜드, XX=제품코드, 8자리=일련번호)
  - QR 코드: 로컬 웹 기반 제품 정보 표시
- **앞면 처리**: 제품별 부품 배치 기준으로 위치 검증
  - 단일 YOLO 모델 (커스텀 데이터셋)
  - 부품 위치 검증 (20px 임계값)
  - 누락/위치오류/추가 컴포넌트 검출
- **데이터셋**: 공개 데이터셋 → 커스텀 PCB 데이터셋 (3종 제품)
- **최종 판정**: 정상/부품누락/위치오류/폐기
- **성능 목표**: 100-150ms (뒷면 50-80ms + 앞면 40-60ms)
- **참고 문서**: `Serial_Number_Detection_Guide.md`, `MySQL_Database_Design.md`

### v2.0 (2025-10-31) ⭐ 이중 모델 아키텍처 (아카이브)
