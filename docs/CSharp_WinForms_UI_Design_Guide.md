# C# WinForms UI 디자인 가이드 ⭐ (이중 모델 아키텍처)
...
**이중 모델 아키텍처 UI 고려사항**:
- ✅ **양면 이미지 표시**: 좌측 (Component), 우측 (Solder)
- ✅ **융합 결과 표시**: 최종 판정 (normal/component_defect/solder_defect/discard)
- ✅ **모델별 상세 정보**: Component 불량 목록 + Solder 불량 목록
- ✅ **성능 모니터링**: 각 모델 추론 시간 + 총 처리 시간
